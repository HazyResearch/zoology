...
from states.utils import import_from_str
...
def init_feature_map(feature_map: str='none', **kwargs: any):
    ...
    if feature_map in [None, 'none', 'identity']:
        from states.mixers.feature_maps.base import FeatureMap
        return FeatureMap(**kwargs)
    # Taylor series approximations to exp(x)
    elif feature_map == 'taylor_exp':
        from states.mixers.feature_maps.taylor import TaylorExp
        return TaylorExp(**kwargs) 
    elif feature_map == "performer":
        from states.mixers.feature_maps.performer import PerformerFeatureMap
        return PerformerFeatureMap(**kwargs)
    elif feature_map == "cosformer":
        from states.mixers.feature_maps.cosformer import CosFormerFeatureMap
        return CosFormerFeatureMap(**kwargs)
    elif feature_map == "pos_elu":
        from states.mixers.feature_maps.base import PosELU
        return PosELU(**kwargs)
    elif feature_map == "all_poly":
        from states.mixers.feature_maps.all_poly import AllPolyMap
