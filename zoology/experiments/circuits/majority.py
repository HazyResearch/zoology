...
from zoology.data.circuits import VocabMajorityConfig
...
SEQLEN=64
VOCAB_SIZE = 64+1
sweep_name = f"redo-vocab-majority-sweep-V{VOCAB_SIZE}" + sweep_id

# 1. First we are going to create the data configuration
train_configs = []
test_configs = []
for k in [2, 4, 8, 16, 24, 32, 64]:
    train_configs.append(VocabMajorityConfig(vocab_size=k+1, input_seq_len=SEQLEN, num_examples=100_000))
    test_configs.append(VocabMajorityConfig(vocab_size=k+1, input_seq_len=SEQLEN, num_examples=1_000))
