...
class TTT(nn.Module):
    """
    Wrapper for TTT (Test-Time Training) layers to match zoology mixer interface.
    """
    def __init__(
        self,
        d_model: int,
        layer_idx: int = 0,
        num_heads: int = 4,
        ttt_layer_type: str = "linear",  # "linear" or "mlp"
        ...
        **kwargs
        ...
        # Create a config object for TTT
        config = TTTConfig(
            hidden_size=d_model,
        ...
        if ttt_layer_type == "linear":
            self.ttt_layer = TTTLinear(config=config, layer_idx=layer_idx)
        elif ttt_layer_type == "mlp":
            self.ttt_layer = TTTMLP(config=config, layer_idx=layer_idx)
        else:
    ...
    def forward(
        ...
        **kwargs
        ...
        # Replicate position_ids generation from TTTModel.forward
        seqlen_offset = 0  # No cache in zoology usage
        position_ids = torch.arange(
            seqlen_offset,
            seqlen_offset + hidden_states.shape[1],
            dtype=torch.long,
            device=hidden_states.device,
        ).unsqueeze(0)
        
        output = self.ttt_layer(
            hidden_states=hidden_states,
            attention_mask=None,
            position_ids=position_ids,
            cache_params=None,
    ...
    def state_size(self, sequence_length: int = 2048) -> int:
        """Return number of scalars in recurrent state (the W matrices)."""
        head_dim = self.d_model // self.num_heads
        
        if self.ttt_layer_type == "linear":
            # W1: [nh, f, f] + b1: [nh, 1, f]
            return self.num_heads * head_dim * (head_dim + 1)
        else:  # mlp
            # W1: [nh, f, 4f] + b1: [nh, 1, 4f] + W2: [nh, 4f, f] + b2: [nh, 1, f]
            return self.num_heads * (8 * head_dim * head_dim + 5 * head_dim)
